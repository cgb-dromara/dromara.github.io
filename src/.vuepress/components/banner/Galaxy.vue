...
const setup = () => {
    ...
    uniforms: {
      ...
      uMouse: {
        value: new Float32Array([
          smoothMousePos.value.x,
          smoothMousePos.value.y,
        ]),
      },
  ...
  function update(t: number) {
    ...
    smoothMousePos.value.x +=
      (targetMousePos.value.x - smoothMousePos.value.x) * lerpFactor;
    smoothMousePos.value.y +=
      (targetMousePos.value.y - smoothMousePos.value.y) * lerpFactor;

    smoothMouseActive.value +=
      (targetMouseActive.value - smoothMouseActive.value) * lerpFactor;
